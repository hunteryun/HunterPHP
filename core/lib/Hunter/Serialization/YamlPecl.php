...
namespace Hunter\Core\Serialization;

use Hunter\Core\Serialization\Exception\InvalidDataTypeException;
...
class YamlPecl implements SerializationInterface {
  ...
  public static function decode($raw) {
    ...
    $data = yaml_parse($raw, 0, $ndocs, [
      YAML_BOOL_TAG => '\Hunter\Core\Serialization\YamlPecl::applyBooleanCallbacks',
    ]);
  ...
  /**
   * Handles errors for \Hunter\Core\Serialization\YamlPecl::decode().
   *
   * @param int $severity
   ...
   *
   * @see \Hunter\Core\Serialization\YamlPecl::decode()
   */
